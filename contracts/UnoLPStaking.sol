...
contract UnoLPStaking {
    ...
    IERC20 public UNOToken;

    address public rewardPool;
    address public owner;

    uint256 public rewardPerBlock;//9 decimals.
    uint256 public accUNOPerShare;
    uint256 public lastRewardBlock;
    uint256 public totalStaked;

    mapping(address => User) public userDetails;

    // Constructor will set the address of UNO/ETH LP token
    constructor(address _LPToken, address _UNOToken, address _rewardPool, uint256 _rewardPerBlock, uint _blocksToWait) {
        LPToken = IERC20(_LPToken);
        UNOToken = IERC20(_UNOToken);
    ...
    // Sets UNO reward for each block
    ...
    function pendingRewards(address _staker) external view returns(uint256) {
        ...
        uint256 _accUNOPerShare = accUNOPerShare;

        if (block.number > lastRewardBlock && totalStaked != 0) {
            uint256 blocksToReward = block.number.sub(lastRewardBlock);
            uint256 unoReward = blocksToReward.mul(rewardPerBlock); // 9 decimals
            _accUNOPerShare = _accUNOPerShare.add(unoReward.mul(1e18).div(totalStaked));
        }

        return user._LPDeposited.mul(_accUNOPerShare).div(1e18).sub(user._rewardDebt);
    }

    // Function that updates UNO/BUSD LP pool
    function updatePool() public returns ( bool ) {
        ...
        uint256 unoReward = blocksToReward.mul(rewardPerBlock);
        accUNOPerShare = accUNOPerShare.add(unoReward.mul(1e18).div(totalStaked));

        UNOToken.safeTransferFrom(rewardPool, address(this), unoReward);

        emit PoolUpdated(blocksToReward, unoReward, block.timestamp);

        return true;
    }

    // Function that lets user stake UNO/BUSD LP
    function stakeLP(uint256 _amount) external returns ( bool ) {
        ...
        if(user._LPDeposited > 0) {
            uint256 _pendingRewards = user._LPDeposited.mul(accUNOPerShare).div(1e18).sub(user._rewardDebt);

            if(_pendingRewards > 0) {
                UNOToken.safeTransfer(msg.sender, _pendingRewards);
        ...
        user._rewardDebt = user._LPDeposited.mul(accUNOPerShare).div(1e18);
    ...
    // Function that lets user unstake UNO/BUSD LP in system
